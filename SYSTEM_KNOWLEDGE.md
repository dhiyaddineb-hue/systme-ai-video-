...
### 4.5 قاعدة الجملة-مشهد (2026-09-14 — طلب المستخدم)
- **كل جملة = مشهد خاص بها**: التقسيم على `[.؟?!]` عبر `scriptwriter.split_sentences` — كل وحدة = لوحة + مقطع صوتي + كابشن مستقل.
- `…` تبقى **داخل** الجملة (وقفة نطق طبيعية) ولا تُقسّم عليها.
- TTS لكل جملة على حدة (قياس حقيقي لكل مشهد) — عدد المقاطع ≈ 2.5× عدد النبضات.
- إيقاع الملخصات: `base 6s` للمشهد (الأسئلة 4s، الختام 3.5s) + `pad = dur+2` + فجوة 0.4s + lead أول 2.0s.
- الجملة الأخيرة ("يتبع") = مشهد الختام فوق بطاقة النهاية.
- التنفيذ: `package_manga_recap_pilot/sentences.json` + `build_sentences.py` (v2 من الحلقة التجريبية).

### 4.6 قاعدة الكلمة-إيقاع + sprite sheets (2026-09-14 — طلب المستخدم "احترافي جداً")
- **كل كلمة تحرّك الإيقاع**: الجملة تُقاس حقيقياً، وتُوزّع على كلماتها تناسبياً (`tts.word_times` — خطأ <0.3s، مُوثّق كتقدير حتى تتوفر word boundaries).
- **sprite sheets**: صورة واحدة = شبكة 2×2 مشاهد (`sheets/sheetN.jpg`) ← تُقص إلى stills (`manga.extract_regions` بأرباع وهوامش 0.02) ← كل still يعيش في `build/` (مشتق، خارج git).
- **beats بصرية**: تجميع الكلمات حتى ≥1.8s = قَطعة بصرية (تدوير الأرباع + pan متناوب) — ~45 beat للحلقة.
- **كابشن بوب تراكمي** (`captions.ass_wordpop`): كل كلمة تُظهر البادئة حتى لحظتها — محاذاة يمين، تشكيل كل بادئة كاملة (RTL صحيح).
- الصوت (15 جملة) والصورة (~45 beat) **مستقلان** على نفس التايم-لاين: `render.render_wordcut`.
- التغطية المتوقعة 85–92% (كثافة الملخصات) — التنفيذ: `build_wordcut.py` → `sief_eldel_ep1_v3.mp4`.

### 4.7 ما قبل الإنتاج: الستوري بورد (2026-09-14 — "شغل بشري احترافي")
- **التدفق الإلزامي:** قصة ← جُمل ← sheets ← **بورد (لقطات + سياسة تكرار)** ← **تدقيق آلي** ← **اعتماد بشري** ← بناء يقرأ البورد ← رندر.
- **تصنيف اللقطات:** `establishing` تأسيسي / `reaction` انفعالي / `transition` انتقالي (قابلة للتكرار بسقف وفجوة) × `detail` تفصيلي / `key_action` حدث مفصلي (**ممنوع التكرار**).
- **قواعد التكرار:** سقف استخدام لكل still + فجوة زمنية دنيا (6–10s) + ممنوع تكرار نفس المنطقة داخل الجملة الواحدة.
- **التنفيذ:** `vtsys/storyboard.py` (سياسة + توزيع + تدقيق) — الكتالوج لكل حلقة في `make_storyboard.py` — البناء يقرأ `storyboard.json` المعتمد.
- **قانون البوابة:** لا رندر إلا من storyboard معتمد بلا مخالفات (أو بمخالفات مُبررة وموثقة).

### 4.8 المحرك السينمائي (2026-09-14 — "نفس النتيجة لم يتغير شيء" → ترقية مرئية)
- **الدرس:** إعادة التوزيع وحدها غير مرئية — الاحتراف المرئي = حركة + صوت + لون + كابشن حي.
- **حركة نوعية** (`vtsys/dynamics.py`): key_action اقتحام+اهتزاز (crop-shake متلاشٍ) / detail انسحاب كاشف / البقية دفع متدرج + تشبع أعلى للأحداث.
- **تدرج مزاجي** لكل نبضة (colorbalance/eq): دفء/رهبة/انفجار/انكسار/تشويق/توتر.
- **فلاش لحظات القوة**: إطار أبيض 0.1s عند كل key_action.
- **كاريوكي ذهبي** (`captions.ass_kinetic`): الجملة كاملة + الكلمة المنطوقة لحظياً بالذهبي (تشكيل كل جزء وحده + ترتيب بصري).
- **SFX مُخلّقة** (بدون تحميل): whoosh عند كل قطع + impact عند key_action + riser قبل الذروة.
- **تدفق الصوت الاحترافي (درس v5):** الـ whoosh/السرير يخضعان لـ ducking تحت السرد، لكن الـ **impact يُمزج بعده** (يخترق فوق الصوت) — وإلا دُفن. المستويات المُقاسة: whoosh 0.35 (كان 0.12 = -38dB غير مسموع!) / impact 0.5.
- **الرندر** (`render.render_dynamic`): فيديو نوعي + صوت (سرد/سرير/SFX) + بطاقات — ترميز واحد.
- **قانون البوابة مُنفّذ كودياً**: `build_cine.py` يرفض العمل إن لم يكن `storyboard.json` معتمداً بلا مخالفات.
