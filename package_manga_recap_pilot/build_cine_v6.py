#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""بناء v6: المشهد الدلالي بالشخصية المقفلة (§4.9).
الاستخدام: python3 package_manga_recap_pilot/build_cine_v6.py
- يعيد استخدام توقيت/أنواع/مزاج البورد المعتمد (البوابة §4.7 تبقى).
- يستبدل الـ stills: كل جملة لها صورتها البطلة panels_v6/sNN.jpg المولّدة من
  محتواها الحرفي بنفس الشخصية (CHARACTERS_v6.md + ref_karim.jpg).
- كل beats الجملة تقصّ من صورتها (قاعدة «عدة مشاهد في صورة واحدة»).
المخرج: sief_eldel_ep1_v6.mp4
"""
...
shots = board["shots"]
print(f"📋 بورد معتمد: {len(shots)} لقطة ✅ (التوقيت) + أبطال v6 (الصور)")

# فشل سريع إن نقصت صور الأبطال
for u in units:
    p = os.path.join(PKG, "panels_v6", f"s{u['id']:02d}.jpg")
    assert os.path.exists(p), f"⛔ صورة البطلة ناقصة: {p}"
print("🖼️ أبطال v6: 15/15 ✅")
...
def hero_of(shot):
    return os.path.join(PKG, "panels_v6", f"s{shot['sent']:02d}.jpg")
...
ass = captions.ass_kinetic(os.path.join(PKG, "manga_v6.ass"), pop, 0.0)
...
out = os.path.join(PKG, "sief_eldel_ep1_v6.mp4")
render.render_dynamic(ff, [hero_of(s) for s in shots], v_sched, a_sched,
